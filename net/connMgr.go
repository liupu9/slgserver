...
// Mgr 管理器
type Mgr struct {
	cm        sync.RWMutex     // 连接管理器锁
	um        sync.RWMutex     // 用户管理器锁
	rm        sync.RWMutex     // 角色管理器锁
	connCache map[int64]WSConn // 连接管理器
	userCache map[int]WSConn   // 用户管理器
	roleCache map[int]WSConn   // 角色管理器
}
